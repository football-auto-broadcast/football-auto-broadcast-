...
### 9. 文件路径契约
### 9.0 本地开发环境路径映射 (重要)
**为了解决 Windows 开发环境下路径不一致的问题，特约定如下映射关系：**

*   **虚拟路径 (Docker/Linux)**: `/mnt/shared`
*   **物理路径 (组长 PC4 节点)**: `\\10.122.241.90\Football_Storage`
*   **使用要求**: 模块 A 和 C 在本地开发联调阶段，必须将输出路径硬编码或配置为上述局域网共享路径。模块 A 在正式启动录制前，需通过访问模块 B 的 `status` 接口确认存储路径的可写权限，防止因权限不足导致的数据丢失。
