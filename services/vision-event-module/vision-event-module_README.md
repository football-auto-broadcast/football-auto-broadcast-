...
A 模块本机 RTSP 视频流
  -> YOLO 实时检测足球/球员基础目标
  -> 提取球坐标、运动强度、禁区活动等视觉信号
  -> 送入 vision-event-module 事件与导播规则
  -> 输出候选事件、双机位关注区域、多机位导播决策
```

其中 A 模块仍负责海康相机接入与 RTSP 分发；C 模块只消费标准化视频流，不直接绑定工业相机 SDK。
...
## 0. 实时视频检测链路
### 视频输入
视觉模块从配置中的两路本机 RTSP 读取视频：

1. `cam_01`: `rtsp://127.0.0.1:8554/main`
2. `cam_02`: `rtsp://127.0.0.1:8555/aux`

该输入来自 `ingest-streaming-module`，冻结规格为 `1920x1080 @ 25fps`。

### YOLO 实时检测
每路视频帧进入视觉模块后，先执行 YOLO 足球/球员基础检测。第一版事件与关注区域输出优先使用以下检测信号：

1. 足球中心点 `ball_x` / `ball_y`
2. 足球检测置信度 `ball_confidence`
3. 运动聚集区域 `motion_cluster`
4. 画面整体活跃度 `global_activity`
5. 门前/禁区 ROI 活跃度 `box_activity`

足球中心点在进入关注区域与事件规则前，会按机位分别经过 Kalman 滤波平滑，降低单帧误检、抖动和短时丢检对导播结果的影响。

### 信号到输出的映射
实时检测信号不会改变对外 JSON 契约，只作为内部依据：

1. `ball_confidence` 高时，关注区域 `source_type` 输出为 `ball_detection`
2. 运动聚集更明显时，关注区域 `source_type` 输出为 `motion_cluster`
3. 检测不可用或单路断流时，仅该路回退 `default`
4. 候选事件仍只输出 4 类冻结事件
5. 多机位决策仍只输出冻结的 `reason` 枚举
